...
// EMERGENCY ALERT COOLDOWN (5-minute cooldown for 0.0 NTU emergency alerts)
#define EMERGENCY_ALERT_COOLDOWN 300000  // 5 minutes = 300000ms
...
// Reference: 2.000 V corresponds to 8.3 mg/L (assume linear with 0V ≈ 0 mg/L)
...
//#define USER_WIFI_SSID "PLDTHOMEFIBREU7zY"
//#define USER_WIFI_PASSWORD "PLDTWIFIzkYb6"
...
unsigned long lastEmergencyAlert = 0;  // Track last emergency alert for 0.0 NTU
bool emergencyAlertSent = false;       // Track if emergency alert was sent for current 0.0 NTU condition
...
void setup() {
  ...
  Serial.println("🔍 Verifying required libraries...");
  Serial.println("✅ ESP32Servo.h - Loaded");
  Serial.println("✅ WiFi.h - Loaded");
  Serial.println("✅ Wire.h - Loaded");
  Serial.println("✅ LiquidCrystal_I2C.h - Loaded");
  Serial.println("✅ DallasTemperature.h - Loaded");
  Serial.println("✅ OneWire.h - Loaded");
  Serial.println("✅ ArduinoJson.h - Loaded");
  Serial.println("✅ HTTPClient.h - Loaded");
  Serial.println("✅ Preferences.h - Loaded");
  Serial.println("✅ SPIFFS.h - Loaded");
  Serial.println("✅ All required libraries verified successfully!");
  ...
  Serial.println("🔧 Applied turbidity calibration (forced): vClear=" + String(turbidityVoltageClearCal, 3) +
  ...
  if (TURBIDITY_CALIBRATION_MODE) {
    Serial.println("🔧 CALIBRATION MODE ENABLED - Running turbidity sensor calibration...");
    
    // First, run diagnostic to check if sensor is working
    Serial.println("\n🔍 RUNNING SENSOR DIAGNOSTIC FIRST...");
    ...
    Serial.println("\n🧪 Testing water condition detection with new calibration...");
  ...
  if (QUICK_TEST_MODE) {
    Serial.println("⚡ QUICK TEST MODE ENABLED - Testing sensor response every 2 seconds...");
  ...
  feedingServo.write(0);
  delay(1000);
  Serial.println("✅ Servo motor test completed");
  ...
  Serial.println("🧪 Testing automated feeding system...");
  ...
  lastSMSAlert = millis();
  lastEmergencyAlert = millis(); // Initialize emergency alert timing
  emergencyAlertSent = false;    // Initialize emergency alert flag
...
void loop() {
  ...
  if (currentTime - lastSensorRead >= SENSOR_READ_INTERVAL) {
    ...
    if (wifiConnected) {
      ...
      Serial.println("⚠️ Skipping Firebase upload - no WiFi connection");
    }
  }
  ...
  if (currentTime - lastGSMCheck >= 60000) { // Check every minute
    if (!gsmReady) {
      Serial.println("🔄 GSM not ready, attempting re-initialization...");
      initializeGSM();
    }
...
void setupWiFiAndDatabase() {
  Serial.println("🔄 Setting up WiFi and database configuration...");
  ...
  if (attemptWiFiConnection(String(USER_WIFI_SSID), String(USER_WIFI_PASSWORD), "user credentials")) {
    ...
    Serial.println("❌ Failed to connect using user credentials");
    wifiConnected = false;
    safeMode = true;
  }
}
...
bool attemptWiFiConnection(const String& ssid, const String& password, const String& connectionType) {
  Serial.println("🔗 Attempting " + connectionType + " connection...");
...
void fetchDatabaseConfiguration() {
  if (!wifiConnected) {
    Serial.println("❌ Cannot fetch database configuration - no WiFi connection");
    return;
  }
  
  if (databaseFetched) {
    Serial.println("ℹ️ Database configuration already fetched this boot cycle");
    return;
  }
  
  Serial.println("📡 Fetching configuration from Firebase database (mobile number fetch disabled)...");
    ...
    if (!error) {
      ...
      if (doc.containsKey("wifiCredentials")) {
        ...
        if (firebaseWiFiSSID.length() > 0 && firebaseWiFiPassword.length() > 0) {
          Serial.println("ℹ️ New WiFi credentials available in database (not used for connection)");
        }
      }
      ...
      safeMode = false;
      Serial.println("✅ Database configuration loaded successfully");
      
    } else {
      Serial.println("❌ Failed to parse database JSON response");
      databaseFetchSuccess = false;
    }
  } else {
    Serial.println("❌ Failed to fetch database configuration. HTTP code: " + String(httpResponseCode));
...
float readDOSensor() {
  ...
  for (int i = 0; i < SENSOR_READINGS_COUNT; i++) {
    ...
    float voltage = (rawValue / 4095.0) * 3.3;
    // Linear mapping assuming 0V ≈ 0 mg/L and 2.000V = 8.3 mg/L
...
void processWaterQuality() {
  // CRITICAL: Check for emergency 0.0 NTU condition FIRST (takes precedence)
  if (turbidityNTU == 0.0) {
    handleEmergencyZeroTurbidity();
    return; // Exit early - emergency takes precedence over normal processing
  }
  
  // Reset emergency alert flag when turbidity changes from 0.0 NTU to any other value
  if (emergencyAlertSent && turbidityNTU > 0.0) {
    emergencyAlertSent = false;
    Serial.println("✅ Emergency condition cleared - turbidity reading changed from 0.0 NTU to " + String(turbidityNTU, 1) + " NTU");
    Serial.println("📱 Emergency alert flag reset - system ready to send new emergency alerts if needed");
  }
  ...
  if (decisionTurbidity <= TURBIDITY_DIRTY_MAX) {
    if (!relayActive) {
      Serial.println("🚨 WATER CHANGE START: Condition=" + waterCondition + 
  ...
  if (decisionTurbidity >= TURBIDITY_CLEAR_MIN) {
    if (relayActive) {
      Serial.println("✅ WATER CHANGE COMPLETE: Water is CLEAR. Decision turbidity: " + String(decisionTurbidity, 1) + " NTU");
...
void handleEmergencyZeroTurbidity() {
  ...
  // Log emergency condition to Serial Monitor
  Serial.println("🚨 EMERGENCY: Possible tank leak detected (0.0 NTU)");
  Serial.println("   This indicates: Empty tank, sensor malfunction, or no water in optical path");
  
  // Send emergency SMS alert only once per 0.0 NTU condition
  if (!emergencyAlertSent) {
    ...
    // Mark that emergency alert has been sent for this 0.0 NTU condition
    emergencyAlertSent = true;
    lastEmergencyAlert = currentTime;
    
    Serial.println("📱 Emergency SMS alert sent: Tank leak/sensor failure detected");
    Serial.println("📱 Will wait for turbidity reading to change before sending another alert");
  } else {
    Serial.println("📱 Emergency alert already sent for this 0.0 NTU condition");
    Serial.println("📱 Waiting for turbidity reading to change before sending another alert");
  }
  
  // Note: This emergency condition takes precedence over normal water classification
  // No relay activation/deactivation occurs during emergency state
  Serial.println("⚠️ Emergency state active - normal water processing suspended");
}
...
void activateWaterReplacement() {
  ...
  Serial.println("💧 Activating water replacement (WATER CHANGING STARTED)...");
...
void deactivateWaterReplacement() {
  ...
  Serial.println("💧 Deactivating water replacement (WATER CHANGING COMPLETED)...");
  Serial.println("   Condition: " + getWaterCondition() + ", Turbidity: " + String(turbidityNTU, 1) + " NTU");
...
void controlRelay() {
  ...
  // Relay turns OFF when water is CLEAR (≤ SAFE threshold)
...
void controlRelay2() {
  ...
  if (!relay2Active) {
    ...
    if (currentTime - relay2StartTime >= RELAY2_OFF_DURATION) {
      ...
      Serial.println("🔄 Relay2 activated - 30 minutes ON cycle started");
    ...
    if (currentTime - relay2StartTime >= RELAY2_ON_DURATION) {
      ...
      Serial.println("🔄 Relay2 deactivated - 30 minutes OFF cycle started");
...
String getWaterCondition() {
  // CRITICAL: Emergency 0.0 NTU condition takes precedence
  if (turbidityNTU == 0.0) {
...
void sendSMSAlert(const String& message) {
  ...
  for (int attempt = 1; attempt <= 3; attempt++) {
    ...
    if (!waitForChar('>', 5000)) {
      Serial.println("❌ SMS attempt " + String(attempt) + ": No prompt '>'");
      continue;
    }
    ...
    if (resp.indexOf("+CMGS:") != -1 && resp.indexOf("OK") != -1) {
      Serial.println("✅ SMS sent successfully to: " + String(ALERT_MOBILE_NUMBER));
      return;
    } else {
      Serial.println("❌ SMS attempt " + String(attempt) + " failed. Response: " + resp);
      delay(1000);
    }
  }

  Serial.println("❌ Failed to send SMS after retries");
}
...
void sendSMSAlertNoBlock(const String& message) {
  // Ensure GSM is initialized
  if (!gsmReady) {
    Serial.println("📱 SMS: GSM not ready, skipping SMS response");
    return;
  }

  Serial.println("📱 Attempting to send SMS response: " + message);

  // Try to send SMS with shorter timeout and single attempt
  gsmSerial.println("AT+CMGF=1");
  if (!waitForSubstring("OK", 1000)) {
    Serial.println("📱 SMS: Failed to set text mode, skipping response");
    return;
  }
  ...
  if (!waitForChar('>', 3000)) {
    Serial.println("📱 SMS: No prompt '>', skipping response");
    return;
  }
  ...
  if (resp.indexOf("+CMGS:") != -1 && resp.indexOf("OK") != -1) {
    Serial.println("✅ SMS response sent successfully");
  } else {
    Serial.println("⚠️ SMS response may have failed, but command was executed");
  }
}
...
void checkAutomatedFeeding() {
  ...
  if (currentHour == MORNING_FEED_HOUR && currentMinute == MORNING_FEED_MINUTE) {
    if (!lastFeedingMorning) {
      Serial.println("🌅 Morning feeding started...");
      performFeeding();
  ...
  if (currentHour == EVENING_FEED_HOUR && currentMinute == EVENING_FEED_MINUTE) {
    if (!lastFeedingEvening) {
      Serial.println("🌙 Evening feeding started...");
      performFeeding();
...
void initializeGSM() {
  ...
  Serial.println("✅ GSM: AT command successful");
  ...
  if (cpin.indexOf("READY") == -1) {
    Serial.println("⚠️ GSM: SIM not ready - Response: " + cpin);
  } else {
    Serial.println("✅ GSM: SIM is ready");
  }
  ...
  if (!registered) {
    Serial.println("⚠️ GSM: Not registered to network");
  }
  ...
  if (smsMode.indexOf("OK") == -1) {
    ...
    Serial.println("✅ GSM: SMS text mode set");
  }
  ...
  Serial.println("✅ GSM: Module ready for SMS sending");

  gsmReady = ok && registered; // Mark ready only if both AT and network registration succeeded
  Serial.println(gsmReady ? "✅ GSM module initialized and ready for SMS" : "❌ GSM module initialized with critical issues");
}
...
void sendSensorDataToFirebase() {
  ...
  if (httpResponseCode == 200) {
    ...
    Serial.println("❌ Failed to send sensor data to Firebase. HTTP code: " + String(httpResponseCode));
    http.end();
  }
  ...
  if (httpResponseCode == 200) {
    Serial.println("✅ Current water condition updated");
  } else {
    Serial.println("⚠️ Failed to update current water condition. HTTP: " + String(httpResponseCode));
  }
  http2.end();
}
  ...
  switch (screenType) {
    ...
    case 1: // Screen 2: Water Condition
      ...
      if (condition.length() > 16) {
        // Truncate long emergency messages for LCD
...
void calibrateTurbiditySensor() {
  Serial.println("🔧 TURBIDITY SENSOR CALIBRATION MODE");
  ...
  Serial.println("");
  Serial.println("⚠️  NOTE: Your sensor shows very low voltages (0.27-0.29V range)");
  ...
  if (fabs(voltageSpan) < 0.005) {
    Serial.println("⚠️  WARNING: Extremely small voltage difference detected!");
    ...
    Serial.println("ℹ️  Very small voltage difference detected - using enhanced sensitivity mapping.");
    Serial.println("   The system will be sensitive to small changes.");
  } else if (fabs(voltageSpan) < 0.05) {
    Serial.println("ℹ️  Small voltage difference detected - using enhanced sensitivity mapping.");
  } else {
    Serial.println("✅ Good voltage difference detected - using standard mapping.");
  }
  ...
  preferences.putFloat("turbidity_v_clear", turbidityVoltageClearCal);
  preferences.putFloat("turbidity_v_dirty", turbidityVoltageDirtyCal);
  Serial.println("💾 Saved turbidity calibration to NVS: vClear=" + String(turbidityVoltageClearCal, 4) +
...
void testWaterConditionDetection() {
  Serial.println("\n🔍 TESTING WATER CONDITION DETECTION");
...
void diagnoseTurbiditySensor() {
  ...
  // Analysis
  if (avgRaw == 0) {
    Serial.println("❌ CRITICAL: Sensor reading 0V - Check power and connections!");
    ...
    Serial.println("❌ CRITICAL: Sensor reading max voltage - Check for short circuit!");
    Serial.println("   Possible issues:");
    ...
    Serial.println("⚠️ WARNING: Very little variation in readings - sensor may be stuck");
    Serial.println("   Possible issues:");
...
void checkSensorFailures() {
  ...
  if (!isValidSensorReading(waterTemperature, -40, 80)) {
    ...
    if (tempFailureCount >= 3) {
      sendSMSAlert("🚨 CRITICAL: Temperature sensor failure detected!");
      tempFailureCount = 0;
    }
  } else {
    tempFailureCount = 0;
  ...
  if (!isValidSensorReading(waterPH, 0, 14)) {
    phFailureCount++;
    if (phFailureCount >= 3) {
      sendSMSAlert("🚨 CRITICAL: pH sensor failure detected!");
      phFailureCount = 0;
    }
  } else {
    phFailureCount = 0;
  ...
  if (!isValidSensorReading(dissolvedOxygen, 0, 20)) {
    doFailureCount++;
    if (doFailureCount >= 3) {
      sendSMSAlert("🚨 CRITICAL: DO sensor failure detected!");
      doFailureCount = 0;
    }
  } else {
    doFailureCount = 0;
  ...
  if (!isValidSensorReading(turbidityNTU, 0, 100)) {
    ...
    if (turbidityFailureCount >= 3) {
      sendSMSAlert("🚨 CRITICAL: Turbidity sensor failure detected!");
